...
$AnalyticsTrophy['SubTitle']    = "班级总积分";
$AnalyticsTrophy['TotalScore']  = $rs->fields['NUM'] + 100;
...
//按一级指标统计积分
$sql = "select 一级指标 AS title, Count(积分分值) AS NUM from data_deyu_banji_record where 班级='$班级' $whereSql group by 一级指标 order by 一级指标 asc";
$rs = $db->Execute($sql);
$rs_a = $rs->GetArray();
$Item = [];
$data = [];
...
$AnalyticsTransactionsCard['SubTitle']    = "按一级指标统计 - 积分明细总数";
...
$ApexAreaChart['Title']       = "班级积分之和";
$ApexAreaChart['SubTitle']    = "按天统计班级积分之和";
...
//ApexAreaChart
$sql = "select 积分时间,sum(积分分值) AS NUM from data_deyu_banji_record where 班级='$班级' $whereSql group by 积分时间 order by 积分时间 asc";
$rs = $db->Execute($sql);
$rs_a = $rs->GetArray();
$输出数据 = [];
for($i=0;$i<sizeof($rs_a);$i++) {
    $输出数据[$rs_a[$i]['积分时间']] = $rs_a[$i]['NUM'] + 100;
}
$dataY = [];
$dataX = array_keys($输出数据);
...
$ApexLineChart['Title']         = "班级积分之和";
$ApexLineChart['SubTitle']      = "按天统计班级积分之和";
...
$sql = "select 积分时间,sum(积分分值) AS NUM from data_deyu_banji_record where 班级='$班级' $whereSql group by 积分时间 order by 积分时间 desc limit 7";
$rs = $db->Execute($sql);
$rs_a = $rs->GetArray();
$输出数据 = [];
for($i=0;$i<sizeof($rs_a);$i++) {
...
$AnalyticsWeeklyOverview['Title']         = "班级积分周报";
$AnalyticsWeeklyOverview['SubTitle']      = "最近一周班级积分之和";
...
$AnalyticsPerformance['SubTitle']    = "按一级指标统计班级积分之和";
...
//ApexRadialBarChart
$sql = "select 一级指标,sum(积分分值) AS NUM from data_deyu_banji_record where 班级='$班级' $whereSql group by 一级指标 order by 一级指标 asc limit 5";
$rs = $db->Execute($sql);
$rs_a = $rs->GetArray();
$输出数据 = [];
for($i=0;$i<sizeof($rs_a);$i++) {
