...
//奖杯模块
$sql = "select Count(积分分值) AS NUM from data_deyu_geren_record where 班级='$班级' $whereSql";
$rs = $db->Execute($sql);
...
//按一级指标统计积分
$sql = "select 一级指标 AS title, Count(积分分值) AS NUM from data_deyu_geren_record where 班级='$班级' $whereSql group by 一级指标 order by 一级指标 asc";
$rs = $db->Execute($sql);
$rs_a = $rs->GetArray();
$Item = [];
$data = [];
