...
export const customizeHttp = (targetParams: RequestConfigType, globalParams: RequestGlobalConfigType) => {
  ...
  if (requestContentType === RequestContentTypeEnum.SQL) {
    headers['Content-Type'] = ContentTypeEnum.JSON
    copyData = JSON.parse(JSON.stringify(requestSQLContent)); //深度复制
    if(copyData['sql']) {
      copyData['sql'] = encodeBase64(copyData['sql'])
    }
    switch(copyData['action']) {
      case 'dbsource':
      case 'db':
      case 'ListData':
        delete copyData.sql;
        delete copyData.field;
        delete copyData.table;
        delete copyData.dbName;
        break;
      case 'table':
        delete copyData.sql;
        delete copyData.field;
    ...
    copyData = JSON.parse(JSON.stringify(data)); //深度复制
  }

  try {    
    const preUrl = process.env.NODE_ENV === 'production' && requestOriginUrl? requestOriginUrl.replace("http://localhost/api/goview/","") : requestOriginUrl;
    //console.log("copyData", copyData)
    //console.log("params", params)
    //console.log("requestUrl", requestUrl)
    //console.log("requestOriginUrl", requestOriginUrl)
    const url =  (new Function("return `" + `${preUrl}${requestUrl}`.trim() + "`"))();
